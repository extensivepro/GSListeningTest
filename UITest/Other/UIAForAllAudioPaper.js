...
for (var index = 0; index < target.frontMostApp().mainWindow().tableViews()[0].cells().length; index ++)
{
    var cell = target.frontMostApp().mainWindow().tableViews()[0].cells()[index];
...
else
{
    ...
    for (var index = 0; index < target.frontMostApp().mainWindow().tableViews()[0].cells().length; index ++)
    {
        ...
        if (target.frontMostApp().mainWindow().tableViews()[0].visibleCells().length > 5)
        {
            ...
            target.delay(2);
            if (target.frontMostApp().navigationBar().rightButton().isValid())
            {
                UIALogger.logPass("重做错题");
